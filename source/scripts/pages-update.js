...
async function main(options) {
	...
	const branchName = `gh-pages-update-${hash}`;
	...
	if (process.env.CI && process.env.GH_TOKEN) {
		console.log(`  ${chalk.gray('⧗')}   submitting pull request "${branchName} → gh-pages" via oauth`);
		...
		console.log(`  ${chalk.green('✔')}   submitted pull request "${branchName} → gh-pages" via oauth`);
	} else {
		console.log(`  ${chalk.gray('⧗')}   submitting pull request "${branchName} → gh-pages" via hub`);
		const pr = shell.exec(`hub pull-request -f -m "${title}" -b ${base} -h ${head}`, {silent: true});

		if (pr.code === 0) {
			console.log(`  ${chalk.green('✔')}   submitted pull request "${branchName} → gh-pages" via hub: ${pr.output.split('\n')[0]}`);
		} else {
			console.log(`  ${chalk.red('✖')}   failed to submit pull request "${branchName} → gh-pages" via hub`);
