...
function main() {
	...
	if (!job) {
		console.log(`   ${chalk.green('✔')}   Skipping browser tests, "process.env.TRAVIS_JOB_NUMBER" is not defined.`);
		throw new Error(1);
	}

	if (!isLeader(job)) {
		console.log(`   ${chalk.green('✔')}   Skipping browser tests, job "${job}" is not the leader.`);
		throw new Error(1);
	}
